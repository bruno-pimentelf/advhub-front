...
export function EventCalendar({
  ...
  const handleEventSave = (event: CalendarEvent) => {
    if (event.id) {
      ...
      toast(`Evento "${event.title}" atualizado`, {
        description: format(new Date(event.start), "MMM d, yyyy", { locale: ptBR }),
  ...
  const handleEventDelete = (eventId: string) => {
    ...
    if (deletedEvent) {
      toast(`Evento "${deletedEvent.title}" excluído`, {
        description: format(new Date(deletedEvent.start), "MMM d, yyyy", { locale: ptBR }),
        position: "bottom-left",
      })
    }
  }
  ...
  const handleEventUpdate = (updatedEvent: CalendarEvent) => {
    ...
    toast(`Evento "${updatedEvent.title}" movido`, {
      description: format(new Date(updatedEvent.start), "MMM d, yyyy", { locale: ptBR }),
      position: "bottom-left",
    })
  }
